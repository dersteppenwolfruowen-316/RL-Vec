...
def sample_to_svg(plan, canvas_size=1024):
    ...
    for room_type, color, stroke_w, style in room_types:
        ...
        if layer_parts:
            ...
            lines.append(f'  <path d="{combined}" stroke="{color}" stroke-width="{stroke_w}" style="{style}"/>')
...
def main():
    ...
    for i, plan in enumerate(data):
        ...
        try:
            ...
            png_fail += 1
            if png_fail <= 5:
                print(f"  PNG FAIL: {sid}: {e}")

        if (i + 1) % 2000 == 0:
            print(f"  {i + 1}/{total} (svg_ok={svg_ok} png_ok={png_ok} png_fail={png_fail}) ...")

    print(f"\nDone!")
    print(f"  SVG: {svg_ok} ok, {svg_fail} skip")
    print(f"  PNG: {png_ok} ok, {png_fail} fail")
